...
export async function DELETE(request: NextRequest) {
  try {
    ...
    cartFotos = cartFotos.filter((item) => item.fid !== fid);

    if (cartFotos.length === initialLength) {
      return NextResponse.json(
        { message: `未找到ID为${fid}的图片` },
        { status: 404 }
      );
    }

    return NextResponse.json({ message: "删除成功", fid }, { status: 200 });
  } catch (error) {
